\documentclass[a4paper,12pt,oneside]{report} % définition des différents paramètres. Le dernier peut être de type report ou article (modifie le chapitrage)
	...
	\usepackage{wrapfig} %package nécessaire pour l'insertion d'images2	\usepackage{amsmath} %package mathématiques (fonction \dfrac notamment)
	...
	\usepackage{icomma}
	\usepackage{physics}
	\usepackage[pdftex,outline]{contour} % glow around text*
	\usepackage{fp}
	\usepackage{ifthen}
	\usepackage{gensymb}
	
	\input{./param/external.tex}
	\input{./commun/commun.tex}
	\input{commun/echelleAltitudeMetres.tex}
	\input{commun/pieChart.tex}
	\input{commun/biblio.tex}
	...
	\input{commun/glossaire.tex}
	\input{01-EtudeAeronefs/glossaire.tex}
	\input{02-Navigation/glossaire.tex}
	\input{03-Meteo/glossaire.tex}
	\input{04-Aerodynamique/glossaire.tex}	
...
%}(\textit{\color{blue}#2})}
	...
	\newcommand{\refDrapeau}[2]{\caption[#1 (Source : \cite{#2})]{}}
...
\input{fonts/fonts.tex}
	...
	\section{Préface}	
	Ce document vise à apporter les connaissances de base nécessaires au passage de l'examen BIA. À date (\today), ce document est en cours de rédaction et doit être considéré comme partiel.
	...
	Ces animations ne sont pas supportées par tous les lecteurs PDF. En particulier, les lecteurs intégrés aux navigateurs (Chrome, Firefox, Edge...) ne prennent généralement pas en charge cette lecture. Le fonctionnement des animations a été vérifié sur les lecteurs Adobe Acrobat Reader ainsi que Foxit Reader.
	...
	À ce jour, l'effort est mis sur la rédaction du cours. Le traitement des particularités de mise en page sera réalisé plus tard, une fois la complétude du cours atteinte.
	\fi
	...
	\label{aerodynamique}
	    \input{04-Aerodynamique/00-Introduction.tex}
	...
	\chapter{Fiches pratiques}
		\input{Annexes/tableauDesUnites.tex}
	
		\input{Annexes/tableauAlphabetRadio.tex}
	...
	\chapter{Frise chronologique}
		Les événements présentés ici sont également disponibles de façon visuelle et interactive \href{http://brevet.initiation.aero/friseChronologique.html}{en ligne sur la frise chronologique}\footnote{\url{http://brevet.initiation.aero/friseChronologique.html}}.
		
		\input{Annexes/friseChronologique/legendeLogo.tex}
		
		\input{Annexes/friseChronologique/frise.tex}
	
	\chapter{Extraits de documents}
		\input{Annexes/cartesOaci.tex}
