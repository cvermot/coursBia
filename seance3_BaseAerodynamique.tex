...
\begin{document}
 ...
 \subsection{La portance}
	...
	\begin{frame}{Les forces sur un aéronef}
		\begin{figure}[H]
			\only<1| handout:0>{\includegraphics[scale=1.2]{imgPresentations/forces.pdf}}
  			\only<2>{\includegraphics[scale=1.2]{04-Aerodynamique/img/forces.pdf}}	
	...
	\subsection{La traînée}
	\begin{frame}{Traînée}
	En physique, un phénomène souhaité est souvent accompagné d'un ou plusieurs phénomènes parasites, indésirables.
	
	\pause
	
	En aérodynamique, la portance s'accompagne d'une force qui va résister à l'avancement de l'aéronef : la traînée.
	
	\end{frame}
	
	\begin{frame}{Relation incidence/traînée}
		\begin{figure}[H]
  			\includegraphics[scale=0.80]{04-Aerodynamique/img/portanceTraineeFinesse_trainee.pdf}
  			\legende{Courbe de la traînée en fonction de l'incidence}{img:portanceTraineeFinesse}	
		\end{figure}
	\end{frame}
	...
	~\newline
	avec : \begin{itemize}
	\item $\rho$ : densité de l'air en $kg/m^3$ \pause($\sim 1,2~kg/m^3$)
	...
	Les tourbillons marginaux sont créés par "l'aspiration" de l'air en surpression de l'intrados vers l'air en dépression sur l'extrados
	\pause
	...
	\begin{column}{0.35\textwidth}
		\pause
		En pratique, la finesse représente la capacité à planer d'un aérodyne.
		
		Elle peut donc se calculer en faisant le rapport de la distance parcourue sur la hauteur perdue quand l'avion est en vol plané (moteur coupé).
		
		\pause
		
		Si un avion parcourt 18 km en perdant 2 km de hauteur, alors sa finesse est égale à :
		\pause
		$\dfrac{18}{2} = 9$
	\end{column}
	\end{columns}
	...
	\end{column}	
 	\begin{column}{0.6\textwidth}
	Obtenue en traçant la portance $C_z$ en fonction de la traînée $C_x$. 
	\pause
 	
 		\begin{itemize}
 			\item $A$ : $Cz = 0 \Rightarrow$ portance nulle
 			\item $B$ : traînée $C_x$ minimale
 			\item $C$ : $\dfrac{C_z}{C_x}$ maximum $\Rightarrow$ finesse maximale
	...
	\begin{frame}{Différents types de dispositifs hypersustentateurs}
	\begin{columns}
 	\begin{column}{0.5\textwidth}
		\begin{figure}[H]
	...
	\end{column}	
 	\begin{column}{0.5\textwidth}
		Augmente la courbure, donc le Cz. Augmente énormément la traînée
 	\end{column}
 	\end{columns}
	\end{frame}
	...
	\begin{itemize}
		\item Toute augmentation de la portance d'une aile se fait au prix de l'augmentation de la traînée
		\pause
...
{multipliée par 6}
{multipliée par 12} 
{Expression de la portance : $Z = \dfrac{1}{2}\rho S V^2 Cz$. Le facteur vitesse est au carré, donc si on multiplie V par 3, la portance est multipliée par $3^2 = 9$.}
...
{d'augmenter la vitesse de décrochage pour certaines man\oe uvres}
