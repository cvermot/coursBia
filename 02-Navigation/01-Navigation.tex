\section{Navigation}
	...
	\subsection{Les grands principes de navigation}
		...
		\paragraph{La navigation à vue}
		La technique de navigation à vue est la première la plus simple des techniques de navigation : on se sert de repères au sol pour déterminer un itinéraire entre 2 points. Le pilote utilise des repères naturels (lacs, rivières, cols ou sommets, vallées...) ou artificiels (villes, antennes, routes, voies ferrées...) pour se repérer. Le pilote navigue ainsi de proche en proche. Cette technique est très simple mais nécessite des cartes précises pour être pleinement utilisable. 
		
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.95\linewidth]{02-Navigation/img/cheminementBordeauxMarmande.pdf}
			\legende{Navigation en cheminement entre Bordeaux et Marmande en suivant une voie ferrée, puis une autoroute puis une voie ferrée.}{img:cheminementBordeauxMarmande}
			\end{figure}	
		
		Il faut par ailleurs être ne mesure de voir le repère suivant une fois que l'on atteint un repère (le repère atteint est appelé \gls{point tournant}, car on effectuera souvent un virage pour rejoindre le point suivant s'il n'est pas situé exactement droit devant) pour poursuivre le vol. plus on vol bas, plus les repères devront dont être rapprochés, et une visibilité réduite peut imposer de multiplier le nombre de point de repère à identifier et à rejoindre, sauf à choisir des repères de type linéaires (suivre un trait de côte, une route, une rivière...).
		
		
		
		\paragraph{La navigation à l'estime}
		Cette technique de navigation permet de naviguer entre 2 repères éloignés, sans nécessité de voir le repère suivant une fois le \gls{point tournant} atteint. Pour cela, le pilote calcule le cap à suivre et la distance le séparant du point suivant. Cette distance lui donne un temps de vol. Il suffit au pilote de voler au cap sur la durée calculée pour atteindre le point suivant.
		
		Il faut bien sur tenir compte du vent autant sur la durée que sur le cap suivi (cf \ref{chap:navigation:capRouteVent} \nameref{chap:navigation:capRouteVent}).
		
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.95\linewidth]{02-Navigation/img/estimeBordeauxMarmande.pdf}
			\legende{Navigation à l'estime entre Bordeaux et Marmande en prenant les villes de Cestas et Langon en points tournants.}{img:estimeBordeauxMarmande}
			\end{figure}	
		
		Cette technique nécessite de déterminer différents paramètres pour la direction à suivre : routes et caps. Ces notions seront détaillées dans le paragraphe suivant.
		...
		\paragraph{Cap et route : l'influence du vent}\label{chap:navigation:capRouteVent}
			\begin{figure}[H]
			\centering
			\input{02-Navigation/img/influenceOrientationForceVent/ventFaceForceVent.tex}
			\legende{Illustration de l'impact de la force du vent de face sur la vitesse sol}{tikz:influenceOrientationForceVent}
			\end{figure}
			
			\begin{figure}[H]
			\centering
			\input{02-Navigation/img/influenceOrientationForceVent/ventTraversForceVent.tex}
			\legende{Illustration de l'impact de la force du vent de travers sur le cap}{tikz:influenceOrientationForceVent}
			\end{figure}
			
			\begin{figure}[H]
			\centering
			\input{02-Navigation/img/influenceOrientationForceVent/ventTraversOrientationVent.tex}
			\legende{Illustration de l'impact de l'orientation du vent sur le cap}{tikz:influenceOrientationForceVent}
			\end{figure}			
		...
		\paragraph{Déclinaison magnétique}
			...
			Par ailleurs, en raison des mouvements de convection du noyau terrestre, l'angle entre le pôle nord magnétique et le pôle nord géographique varie également dans le temps. Une carte des isogones n'est donc valable que pour une période donnée. La légende des cartes isogoniques donne généralement la valeur de la variation de déclinaison annuelle à appliquer.
		...
		Un aéronef évolue toujours selon des règles partagées. Pour les vols civils, on dit que l'on évolue selon les règles de la \textbf{\acrlong{cag}}	(\acrshort{cag}). La CAG définit 2 grands régimes de vol : le VFR et l'IFR.\\
		...
		Le vol \acrshort{ifr} (\anglais{\acrlong{ifr}} - règles de vol aux instruments) définit les règles et procédures permettant de voler par tout temps, lorsque les conditions \acrshort{vmc} ne sont plus réunies. On se trouve alors en conditions \acrshort{imc} (\anglais{\acrlong{imc}} - conditions de vol aux instruments).
	...
	\subsection{Les outils de navigation}
		...
		\subsubsection{Aides à la navigation}
			...
			\paragraph{Le VOR}
			Le \acrshort{vor} \anglais{\acrlong{vor}} est une station émettrice au sol qui permet a un équipage de mesurer un relevement (= route à suivre pour rejoindre la balise) grâce à un récepteur situé dans le cockpit.
			
			%TODO QDM/QDR, ajout photo VOR
			...
			Le \gls{goniomètre} est un équipement présent au sol, dans les tours de contrôle ou les centres de contrôle en route. Cet instrument indique au contrôleur le relèvement d'une émission radio. Lorsqu'un aéronef émet, cet équipement mesure d'où provient l'émission radio. Cette information est disponible auprès du contrôleur, qui peut la fournir aux équipages à leur demande.
			
			Il s'agit d'un moyen qui tombe peu à peu en désuétude, il nécessite en effet un échange radio entre les équipages et une personne au sol pour chaque nouveau besoin de relèvement par l'équipage. Par ailleurs, il ne permet que de donner une direction et jamais une distance.
