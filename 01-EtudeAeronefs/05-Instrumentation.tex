...
\section{L'instrumentation de bord}
    ...
    \begin{minipage}{.77\textwidth}
		...
		%\centering
			...
			%\legende{Réglage au QFE}{img:altimetrieQnh1000hPaAvecStd}
		%\end{figure}
    \end{minipage}
    &
    \begin{minipage}{.23\textwidth}
...
%\end{table}
	...
	\end{figure}
		...
		\begin{figure}[H]
  		...
  		\legende{Maquette de l'avion sur l'horizon artificiel}{tikz:instrumentsBase}
		\end{figure}
...
\end{table}
		  ...
		  \begin{tabular}{c c}
		    \begin{minipage}{.49\textwidth}
		      \begin{figure}[H]
				...
				\horizon{-20}{10}
				\end{tikzpicture}
		  				\legende{Inclinaison à gauche 20° et montée 10°}{tikz:instrumentsBase}			
				\end{figure}
		    \end{minipage} 
		    &
		    \begin{minipage}{.49\textwidth}
		      \begin{figure}[H]
				...
				\horizon{60}{-10}
				\end{tikzpicture}
		  				\legende{Inclinaison à droite 60° et descente 10°}{tikz:instrumentsBase}		
				\end{figure}
		    \end{minipage}
		  \end{tabular}
		  %\caption{}
	...
	La fonction d'indication d'inclinaison est complémentaire de celle de l'horizon artificiel et peut être utilisée en cas de défaillance de l'horizon.
	...
	\subsection{Les instruments de contrôle de la machine}
		...
		Ces 2 instruments sont essentiellement utilisés pour régler la richesse du mélange air-carburant (voir \ref{chap:etudeAeronef:reglageMelange} \refname{chap:etudeAeronef:reglageMelange}).		
		
		L'indicateur de débit carburant permet de suivre la consommation instantanée du moteur.
		
		L'indicateur EGT (pour \textit{Exhaust gas temperature}) est relié à une sonde qui mesure la température des gaz d'échappement. Cet indicateur est utilisé pour régler le mélange air-carburant.
		...
		\info{Ces indicateurs ne sont pas installés sur tous les avions. Il existe des techniques qui permettent de régler le mélange en vol sans avoir recours à ce type d'instruments.}
		
		\subsubsection{Jauges de carburant}
		La (ou plus souvent les) jauge(s) de carburant permettent de contrôler durant le vol la quantité restante de carburant à bord. Il s'agit d'un indicateur qui doit être suivi de façon régulière, car en cas de surconsommation, l'épuisement du carburant présent à bord aboutira à l'extinction du ou des moteurs.
		
		Il y a généralement plusieurs jauges de carburant car les avions sont souvent équipés de plusieurs réservoirs. Dans l'exemple qui suit, on a une jauge pour chacun des réservoirs présents dans les ailes.	
		...
		Ces indicateurs sont parmi les plus importants pour surveiller le moteur. En effet, un moteur qui fonctionne sans pression d'huile sera très rapidement détruit. L'indicateur de pression d'huile est systématiquement vérifié immédiatement après la mise en route, et une absence de montée de la pression d'huile aboutit toujours à l'arrêt immédiat du moteur pour éviter son endommagement.
		
		L'indicateur de température d'huile permet de vérifier que la température d'huile est comprise dans la plage idéale, avant le décollage mais aussi pendant tout le vol. En effet, pour que la lubrification soit assurée convenablement, l'huile doit se trouver dans une plage de température donnée. Trop froide, l'huile est trop visqueuse, trop chaude, elle est trop liquide.
		\begin{figure}[H]
  			\centering
    			\includegraphics[width=0.4\textwidth]{01-EtudeAeronefs/img/instruments/temperaturePressionHuile.pdf}
  			\legende{Indicateur de température et de pression d'huile}{img:temperaturePressionHuile}
		\end{figure}	
		
		\subsubsection{Génération électrique et dépression}
		Ces indicateurs permettent de contrôler le bon fonctionnement de la génération électrique de bord ainsi que du système de dépression. 
		
		L'ampèremètre permet de vérifier si la batterie est en charge ou en décharge.	Une batterie en décharge indique soit une panne de l'alternateur, soit que la consommation électrique de bord dépasse la capacité de génération de l'alternateur.
		
		Le système de dépression \anglais{vaccum} produit la dépression (via une pompe à dépression actionnée par le moteur) nécessaire au fonctionnement de certains instruments de bord (typiquement, les gyroscopes des horizon artificiels sont actionnés grâce à cette dépression). L'indicateur VAC permet de vérifier le fonctionnement de la pompe à dépression.
	...
	De plus en plus d'avions sont équipés d'avionique dite "\gls{glasscockpit}". Ce type d'avionique vise à regrouper l'ensemble des instruments sur un ou plusieurs écrans qui occupent alors la majeure partie de l'espace de la planche de bord.
