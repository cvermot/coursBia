...
%	\end{figure}	
	...
	\subsection{Propulseurs à réaction}
		...
		Le turboréacteur est un système de propulsion qui produit une poussée grâce à l'accélération d'un flux d'air entre l'entrée et la sortie du réacteur.
		
		Un réacteur simple est composé :
		\begin{itemize}
			\item d'une entrée d'air
			\item d'un compresseur \anglais{compressor} dont le rôle est de comprimer l'air qui rentre dans le réacteur
			\item d'une chambre de combustion \anglais{combustion chamber} dans lequel le carburant est injecté pour être brulé
		...
		Dans ce type de réacteur, on sépare le flux en 2 parties :
		\begin{itemize}
			\item le flux primaire qui traverse le réacteur,
			\item le flux secondaire, qui contourne le réacteur.
		\end{itemize}
		Dans ce type de réacteur, le flux secondaire est mis en mouvement par une grande hélice carénée à l'avant du réacteur : la soufflante \anglais{fan}, elle même entrainée via l'arbre de turbine.	
		...
		Un réacteur double flux est toujours composé d'un réacteur double corps, car la soufflante est entrainée par l'axe du compresseur basse pression.
		
		%TODO taux de dilution
