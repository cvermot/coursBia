...
Chaque type d'aéronef dispose de propriétés et de contraintes qui lui sont propres. La classification des aéronefs en grands groupes présentant des caractéristiques communes permet de leur assigner des règles spécifiques aisément, qu'elles soient réglementaires (licence de pilote nécessaire, minima météo, zones de vol autorisées...), techniques (fréquences et mode d'entretien, contraintes de conception) ou administratives (immatriculation, assurance...).
...
\subsection{Aérostats}
	...
	Elle est constituée d'un ballon (appelé enveloppe) sous lequel est accroché une nacelle dans laquelle prennent place les passagers et le pilote nommé aérostier. Un brûleur généralement alimenté au gaz permet de chauffer l'air contenu dans le ballon. Le pilote chauffe l'air à l'aide du brûleur pour faire monter la montgolfière. \\
	
	La montgolfière ne dispose d'aucun moyen pour se diriger, elle est entièrement soumise aux vents dans ses déplacements. Cependant, le pilote peut exploiter les variations de sens du vent aux différentes altitudes pour orienter son vol dans une certaine mesure.
	...
	Par rapport à la montgolfière, ce type de ballon permet une autonomie de vol bien plus importante. En effet, il n'y a pas besoin de combustible pour réchauffer le gaz contenu dans le ballon. Ce type de ballon peut donc en théorie rester en l'air sans limite de temps.
	...
	Le dirigeable \anglais{airship} ou ballon dirigeable est un ballon à gaz équipé de systèmes propulsifs lui permettant de se diriger (aussi bien sûr le plan horizontal que vertical). \\
	...
	\histoire{Bien que l'idée du ballon dirigeable soit apparue pratiquement simultanément à l'invention des ballons, le premier vol d'un dirigeable eut lieu en 1852. On le doit, une fois de plus, à un ingénieur français : Henri Giffard}.
...
\subsection{Aérodynes}
	\subsubsection{Aéronef à voilure fixe}
		...
		\paragraph{Avions}
		La mise en mouvement de l'avion \anglais{airplane} est assurée par son ou ses moteurs.\\
		...
		\legende{Le CriCri, l'un des plus petits avions du monde. Ici présenté dans une version à motorisation électrique}{img:CriCri}
		\end{minipage}
		\begin{minipage}[c]{0.5\linewidth}
		\centering
		\includegraphics[width=0.95\linewidth]{01-EtudeAeronefs/img/a380.jpg}
		\legende{L'A380, l'un des plus gros avions du monde}{img:A380}
		\end{minipage}
		\end{figure}
		\end{minipage}
		\end{center}
	...
	Les appareils à voilure tournante \anglais{rotary-wing aircraft}\footnote{également appelés aérogires, terme vieilli} sont des aéronefs dont la sustentation est assurée par la rotation d'un ou plusieurs rotors.
	
		\paragraph{Hélicoptères}
		L'hélicoptère \anglais{helicopter} est un aérodyne dont la sustentation est assurée exclusivement grâce à un rotor entraîné par un moteur.
		
		Les inventeurs qui ont cherché à concevoir les premiers hélicoptères au début du \siecle{20} souhaitaient une machine dont l'usage serait plus souple que les appareils à voilure fixe, en permettant un décollage court voire vertical. \\
	...
	\end{figure}	
		...
		De par leur capacité à évoluer en stationnaire, les hélicoptères sont devenus indispensables pour de nombreuses missions : secours, héliportage, transport, épandage, lutte aérienne contre les incendies de forêt, travaux sur des lignes électriques, prise de vues...\\
	...
	\end{figure}	
		...
		\paragraph{Autogires}
		L'autogire \anglais{autogyro} est un aérodyne dont la sustentation est assurée grâce à un rotor entraîné par le vent relatif. Les autogires doivent donc disposer d'un mode de propulsion (typiquement une hélice entraînée par un moteur à pistons) qui assure la propulsion de l'appareil sur le plan horizontal.
		
		\histoire{L'autogire a précédé l'hélicoptère. Le premier vol d'un autogire a été effectué en 1923 par l'ingénieur espagnol Juan de la Cierva, contre 1936 pour le premier vol réellement contrôlé pour un hélicoptère.}
	...
	\paragraph{Girodynes}
	Le girodyne \anglais{gyrodyne} est en quelque sorte un hybride entre un hélicoptère et un autogire. Le rotor principal est en effet entraîné par le moteur, tandis que la traction est obtenue par un ou plusieurs autres moteurs.
	...
	Cette configuration présente des avantages en termes de consommation (le plan de rotation du rotor de sustentation reste parallèle au déplacement, ce qui réduit la traînée) tout en conservant les possibilités de décollage et d'atterrissage vertical d'un hélicoptère.
	...
	\info{Le parachute, dans sa version initiale (coupole), se trouve à la limite de la définition d'un aéronef. En effet, ce type de parachute ne produit pas de portance (uniquement de la traînée) et sa capacité de man\oe uvre est très limitée. Le parachute sportif moderne génère, lui, de la portance et pas seulement de la traînée.}
...
\subsection{Engins spatiaux}
	...
	\end{figure}
		...
		Les évolutions récentes des fusées poussent vers des ensembles réutilisables afin de contenir les coûts des lancements.	
	...
	\subsubsection{Engins spatiaux}
		...
		Un satellite artificiel est un objet créé par l'Homme et envoyé dans l'espace grâce à un lanceur. Le satellite orbite (tourne) autour d'une planète (la Terre ou Mars, par exemple) ou de l'un de ses satellites naturels (la Lune par exemple).
		
		Les satellites couvrent des missions très variées. Ils peuvent par exemple servir pour la télécommunication (téléphonie ou télévision par satellite), l'observation de la Terre, à des fins scientifiques ou militaires (satellite espion), ou encore servir de télescopes. 
		...
		\legende{Le télescope Hubble}{img:hubble}
		\end{minipage}
		\begin{minipage}[c]{0.5\linewidth}
		\centering
		...
		\histoire{Le premier satellite artificiel, \textbf{Spoutnik 1}, a été lancé par l'URSS en 1957. \\ La France lancera son premier satellite, \textbf{Astérix}, en 1965.}
	...
	\end{figure}
		...
		Les sondes spatiales peuvent prendre des formes très différentes selon que leur mission sera constituée de simples survols du ou des corps célestes étudiés ou si au contraire elles doivent s'y poser.
		...
		\legende{L'atterrisseur martien Curiosity}{img:curiosity}
		\end{minipage}
		\end{figure}
		\end{minipage}
		\end{center}
		
		\histoire{La sonde spatiale la plus éloignée de la Terre est la sonde Voyager 1. Cette sonde, lancée en 1977 est aujourd'hui située à plus de 25 milliards de kilomètres de la Terre, et continue, malgré son âge et la distance, à envoyer des données scientifiques vers la Terre.}
...
Il existe une grande diversité d'ULM. Il est ainsi possible de piloter des ULM aérostats ou aérodynes, à voilures fixes, tournantes ou souples. En France, ils sont classés en 6 catégories. La licence de pilote d'ULM est commune à toutes les classes, cependant, il faut obtenir une qualification propre à chaque classe, qui permet au pilote de se familiariser avec un instructeur aux particularités propres à chaque type d'aéronef. 

	\begin{figure}[H]
  	\centering
    \includegraphics[width=1.0\textwidth]{01-EtudeAeronefs/img/classesUlm.png}
...
Chaque classe dispose de ses propres limitations en termes de masse et puissance maximale. Les ULM sont monoplace ou biplace.
	...
	Cette classe est composée d'aérodynes constitués de chariots sur lesquels est fixée une aile delta. Le nom pendulaire vient du fait que les changements de trajectoires sont obtenus par déplacement du centre de gravité.	
	...
	Cette classe est composée d'aérodynes à voilure fixe, qui s'apparentent à des avions traditionnels. Il est par ailleurs parfois difficile de distinguer un ULM  multiaxe moderne d'un avion biplace.
	
	\info{Aujourd'hui la frontière entre avion et ULM multiaxes est très réduite. À un tel point que certains fabricants d'avion proposent un même modèle sous la réglementation avion ou ULM en fonction de la masse maximale inscrite sur les documents. C'est le cas de l'ULM présenté en photo ci-dessous (commercialisé sous le nom Bristell Classic sous réglementation ULM et Bristel B23 en réglementation avion).}
	...
	Il s'agit d'aérostats dirigeables dont le volume de l'enveloppe est inférieur à $900~m^3$ pour un  ballon à l'hélium et $2000~m^3$ pour les ballons à air chaud.	
...
Il existe également une grande diversité dans les dimensions des drones. Cela va d'appareils mesurant quelques dizaines de centimètres de côté et quelques centaines de grammes à des machines de plusieurs tonnes et de grande envergure.
