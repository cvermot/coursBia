...
\pgfdeclareimage{altFaceHpa1}{01-EtudeAeronefs/img/instruments/alt/alt_face_hpa_1.pdf}
\pgfdeclareimage{altFaceLowHpa1}{01-EtudeAeronefs/img/instruments/alt/alt_face_low_hpa_1.pdf}
...
\pgfdeclareimage{altButton}{01-EtudeAeronefs/img/instruments/alt/alt_button.pdf}
\def\boutonAltimetre{(-2.1,-2.1)}
...
\def\alti#1#2{
	...
	\node[rotate={-25*((#2-28.0)*100)}] at \boutonAltimetre {\pgfbox[center,center]{\pgfuseimage{altButton}}};
}

%altimètre hPa (graduations 925-1105 hPa)
%   -paramètre 1 : altitude en pieds
%   -paramètre 2 : calage en hPa
\def\altiHpa#1#2{
	\fill[transparent] (0,0) circle (3) ;
	\node[rotate=(#2-925)/(1105-925)*360] {\pgfbox[center,center]{\pgfuseimage{altFaceHpa1}}};
	\node {\pgfbox[center,center]{\pgfuseimage{altFace2}}};
  	\node[rotate=-{(Mod(#1/10,10000))*(36/1000)}] {\pgfbox[center,center]{\pgfuseimage{altFace3}}};
  	\node[rotate=-{(Mod(#1,1000))*(360/1000)}] {\pgfbox[center,center]{\pgfuseimage{altHand2}}};
    	\node[rotate=-{(Mod(#1,10000))*(360/10000)}] {\pgfbox[center,center]{\pgfuseimage{altHand1}}};
    	%\node {\pgfbox[center,center]{\pgfuseimage{altCase}}};
	\node {\pgfbox[center,center]{\pgfuseimage{case}}};
	\node[rotate={-25*((#2-925)/(1105-925)*360)}] at \boutonAltimetre {\pgfbox[center,center]{\pgfuseimage{altButton}}};
}

%altimètre hPa (graduations 875-1055 hPa)
%   -paramètre 1 : altitude en pieds
%   -paramètre 2 : calage en hPa
\def\altiHpaLow#1#2{
	\fill[transparent] (0,0) circle (3) ;
	\node[rotate=(#2-925)/(1055-875)*360] {\pgfbox[center,center]{\pgfuseimage{altFaceLowHpa1}}};
	\node {\pgfbox[center,center]{\pgfuseimage{altFace2}}};
  	\node[rotate=-{(Mod(#1/10,10000))*(36/1000)}] {\pgfbox[center,center]{\pgfuseimage{altFace3}}};
  	\node[rotate=-{(Mod(#1,1000))*(360/1000)}] {\pgfbox[center,center]{\pgfuseimage{altHand2}}};
    	\node[rotate=-{(Mod(#1,10000))*(360/10000)}] {\pgfbox[center,center]{\pgfuseimage{altHand1}}};
    	%\node {\pgfbox[center,center]{\pgfuseimage{altCase}}};
	\node {\pgfbox[center,center]{\pgfuseimage{case}}};
	\node[rotate={-25*((#2-925)/(1055-875)*360)}] at \boutonAltimetre {\pgfbox[center,center]{\pgfuseimage{altButton}}};
}

%altimètre
%   -paramètre 1 : nombre de frames
%   -paramètre 2 : altitude de départ
%   -parametre 3 : pression de départ
%	-paramètre 4 : pression d'arrivée
\def\altiAnimReglageQnhHg#1#2#3#4{
	...
	%\newcommand{\nbFramesAnime}{#1}
	\newcommand{\piedsParUniteDePression}{876.37} % 1 inHg = 876,37 pieds
	\newcommand{\altiDebut}{#2}
	\newcommand{\pressionDebut}{#3}
	\newcommand{\pressionFin}{#4}
	\newcommand{\pasPression}{(\pressionFin-\pressionDebut)/\nbFramesAnime}
...
%altimètre en réglage du QNH
%   -paramètre 1 : nombre de frames
%   -paramètre 2 : altitude de départ
%   -parametre 3 : pression de départ
%	-paramètre 4 : pression d'arrivée
\def\altiAnimReglageQnhHpa#1#2#3#4{
	\ifdefined\activeranimations 
	\newcommand{\nbFramesAnime}{#1}
	\else
	\newcommand{\nbFramesAnime}{1}
	...
	\newcommand{\piedsParUniteDePression}{28} % 1 hPa = 28 pieds
	\newcommand{\altiDebut}{#2}
	\newcommand{\pressionDebut}{#3}
	\newcommand{\pressionFin}{#4}
	\newcommand{\pasPression}{(\pressionFin-\pressionDebut)/\nbFramesAnime}
	\begin{animateinline}[autoplay,loop,controls]{20}
	\multiframe{\nbFramesAnime}{i=0+1}{%
	\begin{tikzpicture}
		\altiHpa{(\altiDebut+(\pasPression*\i*\piedsParUniteDePression))}{(\pressionDebut+\pasPression*\i)};
			%\node[rotate={-5.5*\i}] at \boutonAltimetre {\pgfbox[center,center]{\pgfuseimage{altButton}}};
	\end{tikzpicture}
	}
	\end{animateinline}
}
...
%   -paramètre 1 : nombre de frames
%   -paramètre 2 : altitude de départ
%   -parametre 3 : altitude d'arrivée
%   -parametre 4 : QNH séléctionné
\def\altiHpaAnimChangementAlti#1#2#3#4{
	\ifdefined\activeranimations 
	\newcommand{\nbFramesAnime}{#1}
	\else
	\newcommand{\nbFramesAnime}{1}
	...
	\newcommand{\altiDebut}{#2}
	\newcommand{\altiFin}{#3}
	\newcommand{\pasAlti}{(\altiFin-\altiDebut)/\nbFramesAnime}
	\begin{animateinline}[autoplay,loop,controls]{20}
	\multiframe{\nbFramesAnime}{i=0+1}{%
	\begin{tikzpicture}
		\altiHpa{(\altiDebut+(\pasAlti*\i))}{#4};
	\end{tikzpicture}
	}
	\end{animateinline}
}
...
%   -parametre 2 : position bille [-1;1] 
...
\newcommand{\dessinerTdB}[1]{
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[xshift=14cm, yshift=0cm]
            \altiHpa{\tdbAltitude}{\tdbCalageAltitude}
        \end{scope}
